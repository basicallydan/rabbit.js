...
(function(proto) {
  ...
  proto._consumePrivate = function(bindExchange, callback) {
    ...
    queueName = bindExchange.queueName || '';
    durable = bindExchange.durable || false;

    console.log("Connecting to " + exchangeName + " with " + exchangeType + " type for the binding key " + bindingKey + " on queue " + queueName);

    // NB bind is done async.
    function declareExchangeAndBind(q) {
      if (bindExchange) {
        that._connection.exchange(exchangeName, { type: exchangeType, durable : durable },
                                  function (ex) {
    ...
    if (this._privateQueue) {
      ...
      this._connection.queue(queueName, function(q) {
